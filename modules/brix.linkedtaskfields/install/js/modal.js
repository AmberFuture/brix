class BrixLinkedModal {
    ...
    constructor(params = {}) {
        ...
        this.clickOut = params.clickOut ?? false;
    ...
    modalCreate() {
        ...
        dialog.classList.add("brix-linked-modal");
        container.classList.add("brix-linked-modal__container");

        if (!this.modalOpen) {
            dialog.classList.add("brix-linked-modal--nomodal");
        }
        ...
        if (title !== "") {
            ...
            nodeTitle.classList.add("brix-linked-modal__title");
        ...
        if (text !== "") {
            ...
            nodeText.classList.add("brix-linked-modal__text");
        ...
        if (buttons.length > 0) {
            ...
            btnContainer.classList.add("brix-linked-modal__buttons");
            btnContainer.style.cssText = `--brixModalBtnCount: ${buttons.length};`;

            if (btnCenter) {
                btnContainer.classList.add("brix-linked-modal__buttons--center");
            }
        ...
        if (this.modal) {
            if (this.maxWidth && Number(this.maxWidth) > 0) {
                if (this.modalOpen) {
                    ...
                    this.modal.querySelector(".brix-linked-modal__container").style.maxWidth = `${this.maxWidth}px`;
                }
            }
    ...
    events() {
        ...
        if (this.modal) {
            ...
            if (this.modalOpen && this.clickOut) {
                let doc = this.showInParent ? window.parent.document : document;
                doc.addEventListener("click", (e) => {
                    if ($this.modal.open && e.target.classList.contains("brix-linked-modal")) {
                        $this.modal.close();
                    }
                });
            }
